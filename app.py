...
st.caption("集成总揽及近7/15天看板、运营绩效、SKU 帕累托等级划分及渠道地址分析")
...
if uploaded_file is not None:
    ...
    # =========================================================================
    # TAB 1: 核心总看板 (包含全量、近7天、近15天动态切片与环比)
    # =========================================================================
    with tab_total:
        st.header("📊 全局与近7天/近15天销售数据看板")
        ...
        if 'Order Date' in df.columns and not df['Order Date'].isna().all():
            ...
            period_option = st.radio(
                "选择看板时间范围:", 
                ["近 7 天 (Recent 7 Days)", "近 15 天 (Recent 15 Days)", "近 30 天 (Recent 30 Days)", "自定义日期范围 / 全部数据"], 
                horizontal=True
            )
            ...
            if "近 7 天" in period_option:
                ...
                c1, c2 = st.columns(2)
                date_range = c1.date_input("选择自定义日期区间", [min_date, max_date])
                if len(date_range) == 2:
            ...
            # 计算上一周期数据（用于计算环比 Delta）
            if prev_start_date:
                prev_df = df[(df['Order Date'].dt.date >= prev_start_date) & (df['Order Date'].dt.date <= prev_end_date)]
            else:
            ...
            # 环比百分比计算
            sales_delta = f"{((curr_sales - prev_sales)/prev_sales*100):+.1f}% 上周期" if prev_sales > 0 else None
            qty_delta = f"{((curr_qty - prev_qty)/prev_qty*100):+.1f}% 上周期" if prev_qty > 0 else None
            orders_delta = f"{((curr_orders - prev_orders)/prev_orders*100):+.1f}% 上周期" if prev_orders > 0 else None
            aov_delta = f"{((curr_aov - prev_aov)/prev_aov*100):+.1f}% 上周期" if prev_aov > 0 else None

            st.caption(f"当前展示：**{curr_label}**")
            ...
            # -----------------------------------------------------------------
            # 专有模块：近 7 天 vs 近 15 天 关键指标对比精简表
            # -----------------------------------------------------------------
            st.subheader("⚔️ 近 7 天 vs 近 15 天 销售指标快速对比")
    ...
    with tab_op:
        ...
        if '运营' in df.columns:
            ...
            op_summary['销售额占比'] = (op_summary['总销售额'] / tot_sales_all * 100).round(2)
    ...
    with tab_sku_rank:
        ...
        st.caption("根据销售额累计贡献率（ABC 帕累托分类法）自动划分等级：S/A级 (前80%核心爆款)、B级 (80%-95%腰部主力)、C级 (最后5%尾部滞销)")
